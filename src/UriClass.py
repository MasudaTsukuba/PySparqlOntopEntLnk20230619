...
class UriClass:
    ...
    @staticmethod
    def uri2str(uri_input):  # uri to string conversion
        ...
        for match in re.finditer(r'<(.*?)>', uri_input):  # find <...> pattern
            ...
            response = requests.post(f'http://localhost:5001/', json=data)  # ask the server and receive the results in JSON format
    ...
    @staticmethod
    def str2uri(str_input):  # string to uri conversion for the query results
        ...
        response = requests.post(f'http://localhost:5001', json=data)  # send the request and receive the results in JSON format
